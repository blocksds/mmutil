...
int Get_XM_Frequency(s8 relnote, s8 finetune)
{
    double rn = relnote;
    double ft = finetune;
    double middle_c = 8363.0f;
    double freq = middle_c * pow(2.0, (1.0 / 12.0) * rn + (1.0 / (12.0 * 128.0)) * ft);
    return (int)freq;
}
...
int Load_XM_Instrument(Instrument *inst, MAS_Module *mas, u8 *p_nextsample, bool verbose)
{
    ...
    int inst_headstart = file_tell_read();
    int inst_size = read32();
...
*/
        ...
        for (int x = 0; x < nsamples; x++)
        {
            ...
            int samp_headstart = file_tell_read();
